...
} from "@/app/_services/profile.service";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled)
          setError(
            extractErrorMessage(err as any) || "Unable to load your profile.",
          );
      } finally {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (
      ...
      setError(
        "Please complete your department, programme, and level to finish onboarding.",
      );
      return;
    }
    setSaving(true);
    try {
      ...
      if (onboarding)
        router.replace(
          `/${getDashboardRole({ ...authDetails?.user, ...updated }) || "student"}`,
        );
    } catch (err) {
      setError(
        extractErrorMessage(err as any) || "Unable to update your profile.",
      );
    } finally {
      setSaving(false);
    }
    ...
    return (
      <div className="mx-auto max-w-5xl p-6 md:p-8">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <div className="h-130 animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-900" />
        </div>
      </div>
    );
  }
  ...
  const roleLabel =
    getDashboardRole({ ...authDetails?.user, ...profile }) || "student";
  ...
  return (
    <div className="min-h-full bg-slate-50/80 px-4 py-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100 md:px-8 md:py-8">
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-500 dark:text-slate-400">
              {onboarding
                ? "Add a few academic details so IRAAP can personalize your workspace."
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                  A complete profile makes collaboration and supervision easier.
                </p>
              </div>
        ...
        {error && (
          <div className="mt-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-950/20 dark:text-red-300">
            {error}
          </div>
        )}
        {notice && (
          <div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-300">
            {notice}
          </div>
        )}
          ...
          <aside className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-[#1E293B]">
            <div className="flex flex-col items-center text-center">
              ...
              <button
                ...
                className="mt-4 inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold transition hover:bg-slate-50 disabled:opacity-60 dark:border-slate-700 dark:hover:bg-slate-800"
              >
                {uploadingImage ? (
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-[#1E293B] md:p-7">
            <div className="grid gap-5 md:grid-cols-2">
              {[
                [
                  "fullName",
                  "Full name",
                  "Your name as it should appear across IRAAP",
                  true,
                ],
                ...
                [
                  ...
                  "Your official student identifier",
                  false,
                ],
                ...
                <label key={key as string} className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setField(
                        key as keyof UpdateProfilePayload,
                        e.target.value,
                      )
                    }
                    className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-900/40"
              ...
              <label className="space-y-1.5 md:col-span-2">
                ...
                <textarea
                  ...
                  rows={5}
                  maxLength={1000}
                  className="w-full resize-y rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10 dark:border-slate-700 dark:bg-slate-900/40"
                  placeholder="A short introduction about your academic interests or research focus."
                />
            ...
            <div className="mt-7 flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
              {onboarding ? (
                <button
                  type="button"
                  onClick={() => router.replace(`/${roleLabel}`)}
                  className="text-xs font-semibold text-slate-500 hover:text-slate-900 dark:hover:text-white"
                >
                  Skip for now
                </button>
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {saving ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : onboarding ? (
