...
export default function VerifyOtpPage() {
  ...
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem("iraapOtpChallenge") || "null") as OtpChallenge | null;
  ...
  async function resend() {
    ...
    try {
      ...
      const updated: OtpChallenge = { ...challenge, challengeId: data.challengeId };
  ...
  const destinationLabel = challenge.purpose === "SIGNUP" ? "sign up" : "sign in";
    ...
    >
      <form onSubmit={verify} className="w-full space-y-4" noValidate>
        ...
        <div className="space-y-1.5">
          <label htmlFor="otp" className="block text-xs font-semibold text-foreground/80 dark:text-slate-200">
            Verification code
          </label>
          <input
            id="otp"
        ...
        {isPasswordReset && (
          <>
            <div className="space-y-1.5">
              <label htmlFor="new-password" className="block text-xs font-semibold text-foreground/80 dark:text-slate-200">
                New password
              </label>
            ...
            <div className="space-y-1.5">
              <label htmlFor="confirm-password" className="block text-xs font-semibold text-foreground/80 dark:text-slate-200">
                Confirm password
              </label>
              <div className="relative">
                <input
                  ...
                  className="h-10 w-full rounded-lg border border-slate-200 bg-background px-3 pr-10 text-sm text-foreground placeholder:text-muted-foreground outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/10 disabled:opacity-50 dark:border-slate-700"
                />
                <button type="button" onClick={() => setShowConfirmPassword((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground" aria-label={showConfirmPassword ? "Hide password" : "Show password"}>
        ...
        <button
          type="submit"
          disabled={busy || code.length !== 6 || (isPasswordReset && (password.length < 8 || password !== confirmPassword))}
        ...
        >
          {resending ? "Sending new code…" : cooldown ? `Resend available in ${cooldown}s` : "Resend code"}
        </button>

        <Link href={destination} className="block text-center text-xs text-muted-foreground hover:text-foreground">
